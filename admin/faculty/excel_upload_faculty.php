<?php
     include("../connection.php");
use Box\Spout\Reader\ReaderFactory;
use Box\Spout\Common\Type;
 
// Include Spout library
require_once '../spout-2.4.3/src/Spout/Autoloader/autoload.php';
 
// check file name is not empty
if (!empty($_FILES['file']['name'])) {
      
    // Get File extension  'xlsx' to check file is excel sheet
    $pathinfo = pathinfo($_FILES["file"]["name"]);
     
    // check file has extension xlsx, xls and also check
    // file is not empty
   if (($pathinfo['extension'] == 'xlsx' || $pathinfo['extension'] == 'xls')
           && $_FILES['file']['size'] > 0 ) {
         
        // Temporary file name
        $inputFileName = $_FILES['file']['tmp_name'];
    
        // Read excel file by using ReadFactory object.
        $reader = ReaderFactory::create(Type::XLSX);
 
        // Open file
        $reader->open($inputFileName);
        $count = 1;
        $rows = array();
         
        // Number of sheet in excel file
        foreach ($reader->getSheetIterator() as $sheet) {
             
            // Number of Rows in Excel sheet
            foreach ($sheet->getRowIterator() as $row) {
                
                if ($count > 1) {
                    // Data of excel sheet
                    $fname = $row[0];
                    $lname = $row[1];
                    $name=$fname." ".$lname;
	               $email = $row[2];
	               $pass=$row[3];                
                    $mob=$row[4];                 
                    $regyear=0;
                    $stream="NA";
                    $section="NA";
                    echo $name,$email,$pass,$mob."<br>";
                    $sql=mysql_query("INSERT INTO teacher(name,email,pass,mobile,regyear,stream,section) VALUES(UPPER('$name'),'$email','$pass','".$mob."','$regyear','$stream','$section')",$connect);     
                    if (!$sql) {
                             echo "<script>
                              alert('Error Occured!!!Data Already Exist');
                              window.location.href='faculty.php';
                              </script>";
                         }                    
                }
                $count++;
            }
            break;
        }
        
        $reader->close();
      echo "<script>
     alert('successfully added');
     window.location.href='faculty.php';
     </script>";
    } 
    else {
        echo "<script>
          alert('Select A valid EXCEL File');
          window.location.href='faculty.php';
          </script>";
    }
 
} else {
 
          echo "<script>
          alert(' Select Excel File');
          window.location.href='faculty.php';
          </script>";
     
}
?>